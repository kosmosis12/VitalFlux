...
import { View, SavedView } from '../types';
// FIX: Updated icon names (e.g., Play, Trash) to support the installed version of @phosphor-icons/react.
import { ArrowRight, Play, Trash, IconProps } from '@phosphor-icons/react';
import { ResponsiveContainer, AreaChart, Area } from 'recharts';

// --- Sub-components defined within Home.tsx for encapsulation ---

// 1. HomeHero Component
const HomeHero: React.FC = () => (
    <div className="bg-white dark:bg-gray-800 p-8 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm text-center md:text-left">
...
const ScenarioCard: React.FC<ScenarioCardProps> = ({ icon: Icon, title, description, targetView }) => {
    ...
    return (
        <div onClick={() => setView(targetView)} className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col">
...
const InsightChip: React.FC<InsightChipProps> = ({ label, value, delta, deltaType, sparkData, color, fill }) => {
    const deltaColor = deltaType === 'increase' 
        ? (color.includes('red') ? 'text-red-500' : 'text-green-500') 
        : (color.includes('red') ? 'text-green-500' : 'text-red-500');
            ...
            <div className="h-16 mt-2 -ml-4 -mr-4 -mb-4">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={sparkData}>
                        ...
                        <Area type="monotone" dataKey="v" stroke={fill} strokeWidth={2} fill={`url(#spark-${label})`} />
                    </AreaChart>
                </ResponsiveContainer>
            </div>
        </div>
...
const SavedViewsList: React.FC = () => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
             ...
             {savedViews.length === 0 ? (
                <p className="text-gray-500 dark:text-gray-400">You have no saved views. Navigate to a page and click the save icon in the header to create one.</p>
             ) : (
                <ul className="space-y-3">
                    {savedViews.map((item: SavedView) => (
                        <li key={item.id} className="flex justify-between items-center bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                            <div>
                                <p className="font-semibold text-gray-800 dark:text-gray-200">{item.name}</p>
                                <p className="text-xs text-gray-500 dark:text-gray-400">Page: {item.view}</p>
                            </div>
                            <div className="flex items-center gap-2">
                            <button onClick={() => setView(item.view)} className="p-2 text-white bg-primary-500 rounded-full hover:bg-primary-600 transition-colors" title="Play">
                                <Play size={16} weight="fill" />
                            </button>
                            <button onClick={() => deleteView(item.id)} className="p-2 text-gray-500 hover:bg-red-100 hover:text-red-600 dark:hover:bg-red-900 dark:hover:text-red-200 rounded-full transition-colors" title="Delete">
                                <Trash size={16} />
                            </button>
                            </div>
                        </li>
